...
from edrivingkit_pkg.msg import KitCmdMsgs, KitFeedMsgs
import sys
import termios
import tty
import select
...
class MainProgNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(KitCmdMsgs, 'edrivingkit_cmd', 10)
    ...
    def set_esteering_angle_curses(self, stdscr):
        ...
        try:
            ...
            if -45.0 <= steer_angle <= 45.0:
                ...
                stdscr.addstr(8, 4, "Steering angle must be between -10.0° and 10.0°.")
                stdscr.refresh()
                time.sleep(1.5)
        except ValueError:
