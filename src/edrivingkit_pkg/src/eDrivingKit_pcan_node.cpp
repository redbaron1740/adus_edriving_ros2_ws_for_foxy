...
#include "edrivingkit_pkg/msg/kit_cmd_msgs.hpp"
#include "edrivingkit_pkg/msg/kit_feed_msgs.hpp"
#include "PCANBasic.h"
...
typedef edrivingkit_pkg::msg::KitCmdMsgs KitCmdMsgs;
typedef edrivingkit_pkg::msg::KitFeedMsgs KitFeedMsgs;
...
public:
  ...
  : Node("edrivingkit_pcan_node"), running_(true), m_bSetup_PID_gain(false), m_bSAS_Zero_set(false)
  {
    ...
    {
      std::cout << " setup eSteering_PID: "<< m_esteering_kp_const << std::endl;
    ...
    else 
    {
        ...
        m_tx_msg_00000003h.ID = eBRAKING_CMD_ID_;
        m_tx_msg_00000003h.MSGTYPE = PCAN_MESSAGE_EXTENDED;
        m_tx_msg_00000003h.LEN = 4;

        m_tx_msg_06900001h.ID = eSTEERING_PID_DET_CMD_ID_; // setup_PID_ID_
        m_tx_msg_06900001h.MSGTYPE = PCAN_MESSAGE_EXTENDED;
        m_tx_msg_06900001h.LEN = 8;
        
        m_tx_msg_000007C0h.ID = eSTEERING_SENSOR_CMD_ZEROSET_ID_; // zero_set_ID_
...
private:
  ...
  {
    static int brake_send_counter = 0;
    ...
    // 브레이크 명령 전송  release, eStop                     // 0x00000000D07E0100  : 0x000000003081FEFF
    uint64_t nBrakingMode = msg.cmd_estop_braking == Pull ? 0xD07E0100 : 0x03081FEFF;

    if( this->m_braking_dir == Opposite ) 
      nBrakingMode = msg.cmd_estop_braking == Pull ? 0x03081FEFF : 0xD07E0100;

    memcpy(m_tx_msg_00000003h.DATA, &nBrakingMode, sizeof(nBrakingMode));

    brake_send_counter++;
    if(brake_send_counter >= 2)
    {
      status = CAN_Write(pcan_handle_, &m_tx_msg_00000003h);
      if (status == PCAN_ERROR_QXMTFULL || status == PCAN_ERROR_XMTFULL) 
      {
        RCLCPP_WARN(this->get_logger(), "CAN TX buffer full, retrying...");
        std::this_thread::sleep_for(std::chrono::microseconds(500));
      }
      brake_send_counter = 0; // Reset counter after sending
    }
  ...
  TPCANMsg 				m_tx_msg_00000003h;   //eBRAKING_CMD_ID_
